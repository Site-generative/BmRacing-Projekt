...
const EditCarForm = () => {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await api.getCarById(Number(id));

        formik.setValues({
          maker: response.data.data.maker,
          type: response.data.data.type,
          note: response.data.data.note === null ? '' : response.data.data.note,
          default_driver_id: response.data.data.default_driver_id,
        });
      } catch (err) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        await api.updateCar(Number(id), values.maker, values.type, values.note, Number(values.default_driver_id));
        toast.success('Auto bylo úspěšně upraveno!');
        navigate('/table-cars');
      } catch (error: any) {
        ...
        setSubmitError(error.response?.data?.message || 'Nepodařilo se upravit auto.');
      }
    },
    validate: (values) => {
      const errors: any = {};
