...
const CreateSeriesForm = () => {
  ...
  useEffect(() => {
    const fetchSeries = async () => {
      try {
        const response = await api.getSeriesById(Number(id));
        formik.setValues({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        console.log(Number(id), values.name, values.year.toString());

        await api.updateSeries(Number(id), values.name, values.year.toString());

        toast.success('Série byla úspěšně upravena!');
        ...
        setSubmitError(error.response?.data?.message || 'Nepodařilo se upravit sérii.');
      }
